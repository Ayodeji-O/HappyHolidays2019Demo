...
InputEventInterpreter.prototype.bindInputEventToFunction = function(eventSource, eventCode, functionOwner, targetFunction) {
	if (validateVar(eventSource) && validateVar(eventCode) && validateVar(functionOwner) &&
		validateVar(targetFunction)) {
		...
		if (validateVar(eventDispatcher)) {
			
			if (!validateVar(this.eventReceiverHandlerSetKeyValueStore[eventDataTypeString])) {
...
InputEventInterpreter.prototype.handleEvent = function(event) {
	if (validateVar(event) && (event.type === Constants.eventTypeScalarInput)) {
		var inputEventDataType = event.detail.inputEventType;
		if (validateVar(this.eventReceiverHandlerSetKeyValueStore[inputEventDataType]) &&
			validateVar(this.eventReceiverHandlerSetKeyValueStore[inputEventDataType][event.detail.inputEventCode])) {
