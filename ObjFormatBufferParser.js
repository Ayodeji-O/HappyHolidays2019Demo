...
function ObjFormatBufferParser(sourceBuffer) {
	...
	this.kCharacterNewLine = "\n";
	this.kCharacterContinuation = "\\";
	this.kCharacterSpace = " ";
...
ObjFormatBufferParser.prototype.initiateParsing = function() {
	...
	if (validateVar(this.sourceBuffer)) {
		...
		var continuationDelineatedBuffer = this.sourceBuffer.split(this.kCharacterContinuation);
		...
		for (var continuationBufferLoop = 0; continuationBufferLoop < continuationDelineatedBuffer.length;
			...
			var currentNewLineDelineatedBuffer = continuationDelineatedBuffer[continuationBufferLoop].split(this.kCharacterNewLine);
...
ObjFormatBufferParser.prototype.parseVertexNormalLine = function (inputLine) {
	var singleVertexNormalCompArray = this.parseGenericValueLine(inputLine);
...
ObjFormatBufferParser.prototype.parseTexCoordLine = function (inputLine) {
	var singleVertexTexCoordArray = this.parseGenericValueLine(inputLine);
...
ObjFormatBufferParser.prototype.parseVertexLine = function (inputLine) {
	var singleVertexCoordArray = this.parseGenericValueLine(inputLine);
...
ObjFormatBufferParser.prototype.parseVertexFaceLine = function (inputLine) {
	var vertexSpecArray = inputLine.split(this.kCharacterSpace);
 ...
 *                           specification set
 */
ObjFormatBufferParser.prototype.parseGenericValueLine = function (inputLine) {
	var coordStrArray = inputLine.split(this.kCharacterSpace);
	var currentCoordSet = []
...
ObjFormatBufferParser.prototype.parseVertexSpecString = function (inputString) {
	...
	// follow the vertex coordintes if colors are specified.
...
ObjFormatBufferParser.prototype.parsedIndexValueToWorkingIndexValue = function(indexValue, dataSpecIndex) {
	...
	if (validateVar(indexValue) && validateVar(dataSpecIndex)) {
		if (indexValue < 0) {
...
ObjFormatBufferParser.prototype.preProcessVertexDefinition = function(vertexDefinition) {
	...
	if ((this.vertexDefinitionProcessor !== null) && validateVarAgainstType(vertexDefinition, VertexDefinition)) {
